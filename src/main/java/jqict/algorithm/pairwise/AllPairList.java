	...
	public ParameterValuePair getParameterValuePairByValueIndex(int index) {
		if (index >= 0 && index < valueCount) {
			return new ParameterValuePair(parameterIds[index],
					parameterNames[index], parameterValues[index]);
		}
		return null;
	}
	...
	public int getValueIndexByParameterValuePair(
		...
		for (int i = 0; i < valueCount; i++) {
			if (parameterIds[i].equals(paramId)
					&& parameterValues[i].equals(paramValue)) {
				return i;
			}
		}
		return -1;
	...
	private void initAvailablePairs(List<Parameter> parameters) {
		...
		for (int i = 0; i < parameters.size() - 1; i++) {
			for (int j = i + 1; j < parameters.size(); j++) {
				pairCount += parameters.get(i).getValues().size()
						* parameters.get(j).getValues().size();
			}
		}
		...
		for (int i = 0; i < parameters.size() - 1; i++) {
			...
			int[] value1Indexs = valueIndexMap.get(Integer.valueOf(i));
			for (int j = 0; j < values1.size(); j++) {
				int index1 = value1Indexs[j];

				for (int k = i + 1; k < parameters.size(); k++) {
					List<String> values2 = parameters.get(k).getValues();
					int[] value2Indexs = valueIndexMap.get(Integer.valueOf(k));

					for (int l = 0; l < values2.size(); l++) {
						int index2 = value2Indexs[l];
						availablePairs[pairIndex] = new Pair(pairIndex, index1,
								index2);
						pairIndex++;
					}
				}
