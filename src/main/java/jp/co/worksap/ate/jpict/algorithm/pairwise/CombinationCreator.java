...
public class CombinationCreator {
	...
	private void useParameterValuePair(int pairIndex) {
		...
		Long param1Id = this.allPairList.getParameterIdByValueIndex(newPair
				.getParam1ValueIndex());
		if (!candidate.getMap().containsKey(param1Id)) {
			candidate.getMap().put(
					param1Id,
					this.allPairList.getParameterValuePairByValueIndex(newPair
							.getParam1ValueIndex()));
		}

		Long param2Id = this.allPairList.getParameterIdByValueIndex(newPair
				.getParam2ValueIndex());
	...
	private void findCandidateCombinationByOrder(int[] order, Pair fixedPair) {
		...
		for (int i = 2; i < order.length; i++) {
			...
			ParameterValuePair newParameterValuePair = this.allPairList
					.getParameterValuePairByValueIndex(bestValueIndex);
			this.candidate.getMap().put(newParameterValuePair.getParameterId(),
			...
			for (Iterator<Map.Entry<Long, ParameterValuePair>> iter = this.candidate
					.getMap().entrySet().iterator(); iter.hasNext();) {
				int valueIndex1 = this.allPairList
						.getValueIndexByParameterValuePair(iter.next()
								.getValue());
				Pair newPair = new Pair(0, valueIndex1, bestValueIndex);
				for (int j = 0; j < pairIndices.length; j++) {
					Pair candidatePair = this.allPairList.getAvailablePairs()[pairIndices[j]];
					if (candidatePair.equals(newPair)) {
						useParameterValuePair(candidatePair.getPairIndex());
						break;
					}
				}
			}
	...
	private int[] shuffleParameterOrders(int length, Pair fixedPair) {
		...
		int param1Index = allPairList.getParameterIndexByValueIndex(fixedPair
				.getParam1ValueIndex());
		int param2Index = allPairList.getParameterIndexByValueIndex(fixedPair
				.getParam2ValueIndex());
